...
## 27/03 ✅ PA1 — Sesión 2.5: Cierre de diseño y sincronización documental

Sesión de revisión y consolidación antes de la construcción real en N8N.
Sin código nuevo — arquitectura consolidada y documentación sincronizada.

---

### Problema resuelto: fricción entre diagrama y schema

El diagrama v4.0 describía el flujo de la Capa 2 usando `rowCount`
como señal de negocio tras el UPSERT. El schema v2.0 había incorporado
la CTE que resuelve el problema de 0 items en N8N, pero el diagrama
no reflejaba ese cambio. Los dos documentos describían el mismo sistema
con vocabulario diferente — fricción real que habría generado confusión
al construir.

Con la CTE, N8N siempre recibe 1 fila con `es_nuevo` (bool) y
`timestamp_creacion`. El `rowCount` desaparece porque es un detalle
de implementación que la CTE abstrae hacia arriba.

---

### Triangulación con Gemini — puntos resueltos

Gemini identificó cuatro puntos en su análisis de los documentos v4.1 / v2.1:

**Type casting de timestamp (válido):** PostgreSQL devuelve
`timestamp_creacion` como string ISO, no como objeto Date. En el Code
node que calcula las horas, la conversión es obligatoria:
`const creacion = new Date(item.json.timestamp_creacion);`
Sin el `new Date()`, la resta devuelve `NaN` y el IF de 24h nunca
funciona. Documentado en schema v2.2 como nota de implementación.

**Registro de duplicados (válido — pregunta de validación):**
Todo segundo intento queda registrado con `tipo_evento: duplicado_detectado`.
El tipo de evento clasifica la intención del sistema. El campo
`horas_desde_creacion` en el JSONB da el contexto exacto: 0.08 horas
indica doble click accidental, 600 horas indica cliente que volvió
semanas después. No se necesita un tipo de evento nuevo — el detalle
JSONB es suficiente para distinguirlos en retrospectiva.
Documentado en diagrama v4.2 como nota explicativa en Capa 5.

**EXECUTIONS_DATA_MAX_AGE (corrección de timing):** Gemini sugirió
configurarlo en Sesión 3. Incorrecto — esa variable va en el
docker-compose del servidor de producción, que corresponde a Bloque 6.
En Sesión 3 trabajamos en local sin compose. Documentado como deuda
técnica en schema v2.2 con bloque de resolución correcto.

**CTE confirmada correcta:** Gemini validó la solución como salto de
calidad de "Intermedio" a "Architect". Sin observaciones técnicas.

---

### Documentos actualizados al cierre

- `diagrama-sistema.md` → v4.2
  - rowCount reemplazado por es_nuevo en Capa 2, tabla de eventos
    y sección de decisiones de diseño
  - Nota de distinción duplicado_detectado vs lead_reactivado añadida
  - Decisión "todo segundo intento queda registrado" documentada

- `schema-postgresql-draft.md` → v2.2
  - Nota de implementación: type casting de timestamp_creacion con new Date()
  - EXECUTIONS_DATA_MAX_AGE añadido a tabla de deuda técnica (Bloque 6)
  - Explicación del COUNT(*) FROM upsert = 0 completada

- `gdpr-data-mapping.md` → v2.0 sin cambios — confirmado limpio

---

### Principio internalizado hoy

Cuando una decisión de implementación afecta a múltiples documentos,
se registra en el documento de menor nivel (schema) y se refleja con
el vocabulario correcto en el de mayor nivel (diagrama).
No se mezclan niveles: el diagrama habla de comportamiento, el schema
habla de cómo se ejecuta ese comportamiento en SQL.

---

### Prerequisito antes de abrir N8N para PA1 Sesión 3

Crear las tablas `leads` y `eventos` en pgAdmin con el schema v2.2
